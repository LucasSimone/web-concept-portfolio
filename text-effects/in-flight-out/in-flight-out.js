 ...
 *
 * Usage: give any element `class="in-flight-out"` with the text as its
 * content (or a `data-text` attribute). This file injects its own CSS and
 * auto-initializes every matching element on load.
 */
(function (global) {
  const STYLE_ID = 'in-flight-out-styles';
  const CSS = `
.in-flight-out {
  display: inline-flex;
  flex-wrap: wrap;
  justify-content: center;
  font-size: var(--font-size, clamp(64px, 13vw, 190px));
  line-height: 0.85;
...
`;
  ...
  InFlightOut.get = function (elOrSelector) {
    const el = typeof elOrSelector === 'string' ? document.querySelector(elOrSelector) : elOrSelector;
    return el ? el.__inFlightOutInstance || null : null;
  };

  InFlightOut.getAll = function (selector = '.in-flight-out') {
    return Array.from(document.querySelectorAll(selector))
      .map((el) => el.__inFlightOutInstance)
      .filter(Boolean);
  };
